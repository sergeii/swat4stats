...
class Command(BaseCommand):
    ...
    def handle(self, size=1000, *args, **options):
        ...
        if options['insert']:
            self.FIX_MAP = stats.models.Round.objects.aggregate(min=Min('map'))['min'] * -1
            self.SERVERS = {
        ...
        qs = stats.models.Round.objects.all()
        count = 0
        total = qs.count()
        size = int(size)
        ...
        try:
            while count < total:
                for round in qs[count:count+size]:
                    if int(round.roundend) == 1392746315:
                        raise self.BreakLoop('stopped at %s' % round.pk)
    ...
    def convert_round(self, round, finished_players, outcome):
        ...
        stats = (
            ...
            ('arrests', 'arrests'),
            ('arrested', 'arrested'),
            ('arrestedvip', 'vip_captures'),
            ('unarrestedvip', 'vip_rescues'),
            ('vipescaped', 'vip_escapes'),
            ('validvipkills', 'vip_kills_valid'),
            ('invalidvipkills', 'vip_kills_invalid'),
        )
        with patch.object(timezone, 'now') as mock:
            ...
            players = []
            for finished_player in finished_players:
                player = tracker.models.Player(
                    game=game,
                    dropped=bool(finished_player.dropped),
                    loadout=tracker.models.Loadout.objects.get_or_create(**get_loadout(finished_player))[0],
                    team=0 if (finished_player.is_vip or finished_player.is_swat) else 1,
                    vip=bool(finished_player.is_vip),
                    alias=tracker.models.Alias.objects.match_or_create(
                        name=tracker.utils.force_name(finished_player.player.name, finished_player.ip),
                        ip=int2ip(finished_player.ip)
                    )[0],
                    ip=int2ip(finished_player.ip)
                )
                for old_name, new_name in stats:
                    setattr(player, new_name, getattr(finished_player, old_name))
                if finished_player.player.name != player.name:
                    logger.warning('the name {} of {} has been replaced with {}'
                        .format(finished_player.player.name, int2ip(finished_player.ip), player.name)
                    )
                players.append(player)
            tracker.models.Player.objects.bulk_create(players)
