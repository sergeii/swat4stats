...
class FuzzyFloat(float):
    ...
    def __new__(cls, number: float, precision: int) -> FuzzyFloat:  # noqa: PYI034
        obj = super().__new__(cls, number)
        obj.precision = precision
        return obj

    def __eq__(self, other: float | FuzzyFloat) -> bool:
        return round(self, self.precision) == round(other, self.precision)

    def __ne__(self, other: float | FuzzyFloat) -> bool:
