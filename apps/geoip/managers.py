...
class IPManager(models.Manager):
    def get_queryset(self) -> QuerySet:
        ...
        return (
            ...
            .annotate(
                length=Case(
                    When(Q(range_to__gt=1), then=F("range_to") - F("range_from")),
                    default=0,
                    output_field=models.IntegerField(),
                ),
                is_fresh=Case(
                    When(Q(date_created__gte=min_freshness_date), then=Value(True)),  # noqa: FBT003
...
class ISPManager(models.Manager):
    ...
    def match_or_create(self, ip_address: str | IPv4Address) -> tuple["ISP", bool]:
        ...
        with transaction.atomic():
            ...
            ip_obj, created = IP.objects.select_related("isp").get_or_create(
                range_from=int(network_data.cidr[0]), range_to=int(network_data.cidr[-1])
            )
            # we performed an extra lookup but the same ip range was resolved
            if not created:
                logger.info(
                    "IP range %s-%s already belongs to %s",
                    network_data.cidr[0],
                    network_data.cidr[-1],
                    ip_obj.isp,
                )
            ...
            if "name" not in loopkup_items:
                ...
                # still with a possibility of creating a brand-new object
            else:
                isp, created = (
                    self.get_queryset()
                    .filter(
                        name__isnull=False, country__isnull=("country" not in loopkup_items)
                    )  # country may be null
    ...
    def _query_ip_address(self, ip_address: IPv4Address) -> WhoisQueryResult:
        ...
        if is_defined:
            return WhoisQueryResult(
                description=defined_network_name, country=None, cidr=IPv4Network(ip_address)
            )
        ...
        try:
            ...
            logger.info(
                "failed to query whois for %s due to %s(%s)", ip_address, type(exc).__name__, exc
            )
            raise
        ...
        if net_country := network_data.get("country"):  # noqa: SIM108
