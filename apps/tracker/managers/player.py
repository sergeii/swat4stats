...
class PlayerQuerySet(models.QuerySet):
    ...
    player_aggregates: ClassVar[list[dict[str, Expression]]] = [
        {
            ...
            "wins": Count(
                Case(When(q_swat_victories | q_sus_victories, then="game")), distinct=True
            ),
            ...
            "spm_ratio": Round(
                as_float(_agg_player_score) / NullIf(_agg_player_time, Value(0)) * Value(60), 4
            ),
            "spr_ratio": Round(
                as_float(_agg_player_score) / NullIf(_agg_player_games, Value(0)), 4
            ),
            ...
            "kd_ratio": Round(
                as_float(_agg_player_kills) / NullIf(_agg_player_deaths, Value(0)), 4
            ),
        },
        {
            ...
            "weapon_hit_ratio": Round(
                as_float(_agg_player_weapon_hits) / NullIf(_agg_player_weapon_shots, Value(0)), 4
            ),
            "weapon_kill_ratio": Round(
                as_float(_agg_player_weapon_kills) / NullIf(_agg_player_weapon_shots, Value(0)), 4
            ),
            "weapon_teamhit_ratio": Round(
                as_float(_agg_player_weapon_teamhits) / NullIf(_agg_player_weapon_shots, Value(0)),
                4,
            ),
            ...
            "grenade_hit_ratio": Round(
                as_float(_agg_player_grenade_hits) / NullIf(_agg_player_grenade_shots, Value(0)), 4
            ),
            "grenade_teamhit_ratio": Round(
                as_float(_agg_player_grenade_teamhits)
                / NullIf(_agg_player_grenade_shots, Value(0)),
                4,
            ),
        },
    ]
    ...
    weapon_aggregates: ClassVar[list[dict[str, Expression]]] = [
        {
            ...
            "kill_ratio": Round(
                as_float(_agg_weapon_kills) / NullIf(_agg_weapon_shots, Value(0)), 4
            ),
            "teamhit_ratio": Round(
                as_float(_agg_weapon_teamhits) / NullIf(_agg_weapon_shots, Value(0)), 4
            ),
        }
    ]
    ...
    def aggregate_player_stats(self) -> dict[str, int | float]:
        return self.filter(q_versus_modes).aggregate_stats_groups(self.player_aggregates)

    def aggregate_stats_by_weapon(self) -> dict[str, dict[str, int | float]]:
        return self.filter(q_versus_modes).aggregate_stats_groups_by(
            self.weapon_aggregates, group_by="weapon__name"
        )
    ...
    def aggregate_stats_groups(
        self, aggregate_groups: list[dict[str, models.Aggregate]]
    ) -> dict[str, int | float]:
        result = {}
    ...
    def aggregate_stats_by(
        self, aggregates: dict[str, models.Aggregate], *, group_by: str
    ) -> dict[str, dict[str, int | float]]:
        result = {}

        prepared_aggregates = self._prepare_aggregates(aggregates)
        grouped_items = self.order_by(group_by).values(group_by).annotate(**prepared_aggregates)
    ...
    def _prepare_aggregates(
        self, aggregates: dict[str, models.Aggregate]
    ) -> dict[str, models.Aggregate]:
        prepared_aggregates = {}
    ...
    def _finalize_aggregates(
        self, aggregated_items: dict[str, int | float | str]
