...
class MotdLeaderboardForm(MotdBaseForm):
    ...
    def clean(self) -> dict[str, Any]:
        ...
        gametype_slug = cleaned_data.get('gametype')
        if gametype_slug:
        ...
        cleaned_data.update({
            ...
            'repeat': self.Defaults.repeat if cleaned_data['repeat'] is None else cleaned_data['repeat'],
        })
...
class APIMotdLeaderboardView(TemplateView):
    ...
    def get_context_data(self, *, form: MotdLeaderboardForm, **kwargs: Any) -> dict[str, Any]:
        ...
        context_data['players'] = self._get_players_for_leaderboard(form.cleaned_data['category'],
                                                                    form.cleaned_data['gametype'],
                                                                    form.cleaned_data['limit'])

        return context_data

    def _get_players_for_leaderboard(self,
                                     category: str,
                                     gametype: str | None,
                                     limit: int) -> list[PlayerStats | GametypeStats]:
        effective_year = get_current_stat_year()

        if gametype:
            qs = (GametypeStats.objects
                  .filter(category=category,
                          gametype=gametype,
                          year=effective_year))
        else:
            qs = PlayerStats.objects.filter(category=category,
                                            year=effective_year)

        qs = (qs
              .filter(position__isnull=False, position__lte=limit)
