...
class APIResponse:
    ...
    @classmethod
    def _make_response(
        cls, status: APIResponseStatus, message: str | list[str] | None
    ) -> HttpResponse:
        parts = [status.value]
...
def require_julia_schema(schema: Callable, schema_error_message: str | None = None) -> Callable:
    ...
    the view returns an error response with the specified error message.
    """
        ...
        def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
            ...
            match request.META.get("CONTENT_TYPE"):
                ...
                case _:
                    ...
                    expand_func = (
                        JuliaQueryString.expand_dots
                        if julia_dotted
                        else JuliaQueryString.expand_array
                    )
