...
class ServerMergeForm(forms.Form):
    ...
    def clean(self) -> dict[str, Any]:
        ...
        if main_server.merged_into is not None:
            raise ValidationError(_('Target server is already merged into %(server)s') % {
                'server': f'{main_server.merged_into.name} ({main_server.merged_into.address})',
            })

        merged_servers = (Server.objects
        ...
        self.cleaned_data.update({
            'main_server': main_server.pk,
            'merged_servers': [s.pk for s in merged_servers]
        })
...
class ServerAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('ip', 'port', 'status_port', 'hostname', 'country', 'enabled', 'listed', 'pinned'),
        }),
        (_('Meta'), {
            'fields': ('version', 'failures',
                       'admin_merged_into', 'admin_merged_with',
                       'merged_into_at', 'merged_stats_at'),
        }),
    )
    readonly_fields = ('version', 'failures',
                       'admin_merged_into', 'admin_merged_with',
                       'merged_into_at', 'merged_stats_at')
    list_per_page = 50
    ...
    def merge_servers(self, request: HttpRequest, queryset: QuerySet):
    ...
    def merge_servers_form(self, request: HttpRequest):
        ...
        if request.method == 'POST' and form.is_valid():
            logger.info('firing merge servers task with main_server_id=%s merged_server_ids=%s',
                        form.cleaned_data['main_server'], concat_it(form.cleaned_data['merged_servers']))
            merge_servers.delay(main_server_id=form.cleaned_data['main_server'],
