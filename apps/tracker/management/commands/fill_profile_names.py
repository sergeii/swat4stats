...
def fill_profile_names(profile_model: type[Model]):
    sub = Alias.objects.distinct("name").filter(profile_id=OuterRef("pk")).values_list("name")
    profile_names = list(
        profile_model.objects.annotate(alias_names=ArraySubquery(sub)).values("pk", "alias_names")
    )

    logger.info("updating denorm fields for %s profiles", len(profile_names))

    for chunk in iterate_list(profile_names, size=1000):
        name_per_profile = {item["pk"]: item["alias_names"] for item in chunk}
        chunk_qs = (
            profile_model.objects.select_related(None)
            .filter(pk__in=name_per_profile)
            .only("pk", "name")
        )
        for profile in chunk_qs:
            names = [name for name in name_per_profile[profile.pk] if name != profile.name]
            profile.names = names
