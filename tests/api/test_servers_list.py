...
def test_get_server_list(db, api_client):
    ...
    vip_server = ServerFactory(
        listed=True, status=ServerStatusFactory(gametype="VIP Escort", numplayers=16, maxplayers=16)
    )
    pinned_server = ServerFactory(
        listed=True,
        pinned=True,
        status=ServerStatusFactory(gametype="VIP Escort", numplayers=16, maxplayers=16),
    )
    vip_server10 = ServerFactory(
        listed=True,
        status=ServerStatusFactory(
            gametype="VIP Escort", numplayers=15, maxplayers=18, gamever="1.0"
        ),
    )
    bs_server = ServerFactory(
        listed=True, status=ServerStatusFactory(gametype="Barricaded Suspects")
    )
    coop_server = ServerFactory(listed=True, status=ServerStatusFactory(gametype="CO-OP"))
    sg_server = ServerFactory(
        listed=True,
        status=ServerStatusFactory(gametype="Smash And Grab", gamevariant="SWAT 4X", gamever="1.0"),
    )
    tss_coop_server = ServerFactory(
        listed=True,
        status=ServerStatusFactory(gametype="CO-OP", gamevariant="SWAT 4X", gamever="1.0"),
    )
    passworded_server = ServerFactory(listed=True, status=ServerStatusFactory(password=True))
    ...
    responses = [
        (
            {"empty": "true"},
            [bs_server.pk, coop_server.pk, sg_server.pk, tss_coop_server.pk, passworded_server.pk],
        ),
        ...
        (
            {"gamever": "1.1"},
            [vip_server.pk, pinned_server.pk, bs_server.pk, coop_server.pk, passworded_server.pk],
        ),
        ({"gamever": "1.0"}, [vip_server10.pk, sg_server.pk, tss_coop_server.pk]),
        ({"gamever": "1.1", "gamename": "SWAT 4X"}, []),
        (
            {"gamever": "1.1", "gamename": "SWAT 4"},
            [vip_server.pk, pinned_server.pk, bs_server.pk, coop_server.pk, passworded_server.pk],
        ),
