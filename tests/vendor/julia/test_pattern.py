...
def sample():
    def opener(filename):
        path = os.path.join(os.path.dirname(__file__), 'sample', filename)
        return open(path)
    return opener


def test_dot_samples(pattern, sample):
    pattern_node = shortcuts.parse_pattern(pattern)
    with sample('dot.txt') as f:
        for value in f:
            value = value.strip()
            if not value:
                continue
            qs = shortcuts.julia_v2(value)
            pattern_node.parse(qs)


def test_array_samples(pattern, sample):
    pattern_node = shortcuts.parse_pattern(pattern)
    with sample('array.txt') as f:
        for value in f:
            value = value.strip()
            if not value:
                continue
