...
def test_merge_action_redirects_to_merge_form(db, admin_client, merge_form_url):
    ...
    resp = admin_client.post(reverse('admin:tracker_server_changelist'),
...
def test_merge_action_no_servers(db, admin_client, changelist_url):
    resp = admin_client.post(reverse('admin:tracker_server_changelist'),
...
def test_render_merge_form_choices(db, admin_client, merge_form_url):
    ...
    assert resp.status_code == 200
    expected_choices = [
        (s.pk, f'{s.hostname} ({s.address})')
        for s in [server3, server2, server1]
    ]
...
def test_submit_merge_form_ok(task_mock, db, admin_client, merge_form_url, changelist_url):
    server1, server2, server3 = ServerFactory.create_batch(3)
    resp = admin_client.post(f'{merge_form_url}?ids={server1.pk},{server2.pk},{server3.pk}',
                             {'main_server': server2.pk,
                              'merged_servers': [server1.pk, server3.pk],
                              'confirm': 'yes'})
    assert resp.status_code == 302
    assert resp.url == changelist_url
    task_mock.assert_called_once_with((), {'main_server_id': server2.pk, 'merged_server_ids': [server1.pk, server3.pk]})
...
def test_cant_merge_server_into_itself(task_mock, db, admin_client, merge_form_url):
    server = ServerFactory()
    resp = admin_client.post(f'{merge_form_url}?ids={server.pk}',
                             {'main_server': server.pk,
                              'merged_servers': [server.pk],
                              'confirm': 'yes'})
    assert resp.status_code == 200
...
def test_cant_merge_into_merged_server(task_mock, db, admin_client, merge_form_url):
    ...
    resp = admin_client.post(f'{merge_form_url}?ids={server1.pk},{server2.pk}',
                             {'main_server': server1.pk,
                              'merged_servers': [server2.pk],
                              'confirm': 'yes'})
    assert resp.status_code == 200
    assert not resp.context['form'].is_valid()
    assert not task_mock.called
...
def test_cant_merge_already_merged_servers(task_mock, db, admin_client, merge_form_url):
    ...
    resp = admin_client.post(f'{merge_form_url}?ids={server1.pk},{server2.pk},{server3.pk}',
                             {'main_server': server1.pk,
                              'merged_servers': [server2.pk, server3.pk],
                              'confirm': 'yes'})
    assert resp.status_code == 200
    assert not resp.context['form'].is_valid()
    assert not task_mock.called
...
@mock.patch.object(merge_servers, 'apply_async')
def test_merge_form_handle_empty_choices(task_mock, db, admin_client, merge_form_url, changelist_url):
    server1, server2, server3 = ServerFactory.create_batch(3)
    resp = admin_client.post(f'{merge_form_url}?ids={server1.pk}',
                             {'main_server': '',
                              'merged_servers': [],
                              'confirm': 'yes'})
    assert resp.status_code == 200
...
@mock.patch.object(merge_servers, 'apply_async')
def test_merge_form_handle_invalid_choices(task_mock, db, admin_client, merge_form_url, changelist_url):
    server1, server2, server3 = ServerFactory.create_batch(3)
    resp = admin_client.post(f'{merge_form_url}?ids={server1.pk}',
                             {'main_server': server1.pk,
                              'merged_servers': [server2.pk, server3.pk],
