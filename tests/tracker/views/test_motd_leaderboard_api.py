...
def test_motd_leaderboard(db, names, client, initial, limit, repeat, delay):
    for i, name in enumerate(names[:7]):
        PlayerStatsFactory(category='score', year=2021, position=i+1,
                           profile__name=name)
...
def test_motd_random_leaderboard(db, names, client):
    for i, name in enumerate(names[7:]):
        PlayerStatsFactory(category='arrests', year=2021, position=i+1,
                           profile__name=name)
    ...
    with freeze_timezone_now(datetime(2021, 5, 29, 12, 17, 1, tzinfo=UTC)), \
        ...
        choice_mock.assert_called_once_with(['score', 'top_score', 'time', 'wins',
                                             'kills', 'arrests',
                                             'top_kill_streak', 'top_arrest_streak',
                                             'spm_ratio', 'spr_ratio',
                                             'kd_ratio', 'weapon_hit_ratio'])
...
@pytest.mark.parametrize('day, month, year, effective', [
    (31, 12, 2020, 2020),
    (1, 1, 2021, 2020),
    (10, 1, 2021, 2020),
    (17, 1, 2021, 2021),
...
def test_motd_leaderboard_select_effective_year(db, names, client, day, month, year, effective):
    for i, name in enumerate(names[:7]):
        PlayerStatsFactory(category='score', year=2020, position=i+1,
                           profile__name=name)
    for i, name in enumerate(names[7:]):
        PlayerStatsFactory(category='score', year=2021, position=i+1,
                           profile__name=name)
...
@pytest.mark.parametrize('alias, category, title', [
    ('score', 'score', 'Score'),
...
def test_motd_leaderboard_categories_compat(db, client, names, alias, category, title):
    for i, name in enumerate(names[:7]):
        PlayerStatsFactory(category=category, year=2021, position=i+1,
                           profile__name=name)
...
@pytest.mark.parametrize('category, gametype, title', [
...
def test_motd_leaderboard_gametype_categories_compat(db, client, names, category, gametype, title):
    for i, name in enumerate(names[:7]):
        GametypeStatsFactory(category=category, year=2021, gametype=gametype,
                             position=i+1, profile__name=name)
