...
from apps.tracker.factories import (
    ServerFactory, GameFactory,
    ServerGameDataFactory, PlayerGameDataFactory, SimplePlayerGameDataFactory,
    AliasFactory, ProfileFactory, WeaponGameDataFactory,
    ProcedureGameDataFactory, ObjectiveGameDataFactory,
    PlayerFactory, LoadoutFactory
)
...
def test_game_data(db):
    return ServerGameDataFactory(tag='foobar',
                                 port=10480,
...
def post_game_data(client):
    ...
    def poster(game_data, *, client=None, **headers):
        ...
        return client.post('/stream/', game_data.to_json(), content_type='application/json', **headers)
    return poster
...
def test_post_game_data(db, post_game_data):
    ...
    assert set(player2.weapons.values_list('name', flat=True)) == {'9mm SMG',
                                                                   'Suppressed 9mm SMG',
                                                                   'Stinger'}
...
def test_post_game_data_with_players(db, whois_mock, post_game_data):
    whois_mock.side_effect = [
        ...
        {'nets': [{'description': 'Another Norwegian ISP', 'country': 'no', 'cidr': '16.16.0.0/16'}]},
    ...
    semi_empty_loadout = LoadoutFactory(primary='M4 Super90', secondary='Taser Stun Gun',
                                        equip_one='Optiwand', equip_two='Flashbang')
    full_loadout = LoadoutFactory(primary='M4 Super90',
    ...
    sco_another_alias = AliasFactory(name='Scofield', isp__name='AnoterScoISP', isp__ip='101.101.101.0/24')  # noqa
    burrows_country_alias = AliasFactory(name='Burrows', isp__ip='200.200.200.0/24',
                                         isp__name='Some Norwegian ISP', isp__country='no')
    PlayerFactory(alias=burrows_country_alias, ip='200.200.200.188')

    heebo_player = PlayerFactory(ip='17.17.17.17',
                                 alias__name='Greebo', alias__isp=None,
                                 game__date_finished=now() - timedelta(days=20))

    prophet_player = PlayerFactory(ip='18.18.18.18',
                                   alias__name='Coagulant', alias__isp=None,
                                   game__date_finished=now() - timedelta(days=365))

    player_isp_alias = AliasFactory(name='Player', isp__name='PlayerISP',
                                    isp__country='gb', isp__ip='19.19.0.0/16')
    ...
    game_data = ServerGameDataFactory(
        ...
        players=[
            ...
            # null loadout
            PlayerGameDataFactory(name='LoadoutPlayer',
                                  ip='1.1.12.124',
                                  loadout={}),
            # missing loadout
            SimplePlayerGameDataFactory(name='NoLoadoutPlayer', ip='1.1.18.12'),
            # semi null loadout
            PlayerGameDataFactory(name='SemiLoadoutPlayer',
                                  ip='1.1.18.19',
                                  loadout={0: 1, 2: 16, 4: 27, 5: 23}),
            # full loadout
            PlayerGameDataFactory(name='FullLoadoutPlayer',
                                  ip='1.1.189.1',
    ...
    (cris, cris2, sinaas, stewie, empty,
     mek, kont, bag,
     loadout_player, no_loadout_player, semi_loadout_player, full_loadout_player,
     sco, burrows, heebo, prophet, player) = players
    ...
    for slot in ['primary', 'primary_ammo', 'secondary', 'secondary_ammo',
...
def test_post_game_data_updates_player_profiles(db, post_game_data):
    ...
    profile3 = AliasFactory(name='Brandon',
                            isp=ISPFactory(ip='5.5.5.0/24'),
                            profile=ProfileFactory(game_first=game3,
                                                   game_last=game3,
                                                   first_seen_at=game3.date_finished,
                                                   last_seen_at=game3.date_finished)).profile
    profile4 = AliasFactory(name='Meera',
                            isp=ISPFactory(ip='10.10.10.0/24'),
                            profile=ProfileFactory(game_first=game4,
                                                   first_seen_at=game4.date_finished,
                                                   game_last=None)).profile

    assert not (profile1.game_last or profile1.game_first or
                profile1.first_seen_at or profile1.last_seen_at)
    assert (profile3.game_first and profile3.game_last and
            profile3.first_seen_at and profile3.last_seen_at)

    game_data = ServerGameDataFactory(players=[
    ...
    assert not (profile2.game_last or profile2.game_first or
                profile2.first_seen_at or profile2.last_seen_at)
...
def test_post_rapid_deployment_game(db, post_game_data, test_server):
    game_data = ServerGameDataFactory(
        ...
        bombs_defused=4,
        bombs_total=5,
        players=(PlayerGameDataFactory.create_batch(2, rd_bombs_defused=2, team=0) +
    ...
    assert list(game.player_set.filter(team='swat')
                .values('rd_bombs_defused')) == [{'rd_bombs_defused': 2},
                                                 {'rd_bombs_defused': 2}]
...
def test_post_vip_escort_game(db, post_game_data, test_server):
    game_data = ServerGameDataFactory(
        ...
        outcome=8,
        player_num=4,
        players=[PlayerGameDataFactory(name='Foo', vip_rescues=2, team=0),
...
def test_post_coop_game(db, post_game_data, test_server):
    game_data = ServerGameDataFactory(
        ...
        players=[PlayerGameDataFactory(name='Foo',
                                       team=0,
                                       coop_status=3,
                 ...
                 PlayerGameDataFactory(name='Ham',
                                       team=0,
                                       coop_status=2,
                                       coop_hostage_arrests=12,
                                       coop_hostage_incaps=1,
                                       ...
                                       coop_toc_reports=5),
                 PlayerGameDataFactory(name='Baz',
                                       team=1,
                                       coop_status=1,
                                       coop_hostage_hits=1,
                                       coop_hostage_incaps=1,
...
def test_post_smash_and_grab_game(db, post_game_data, test_server):
    game_data = ServerGameDataFactory(
        ...
        outcome=13,
        player_num=3,
        players=[PlayerGameDataFactory(name='Foo', team=0),
...
@pytest.mark.parametrize('data_encoder, content_type', [
...
def test_various_data_forms_are_accepted(db, data_encoder, content_type, client):
    ...
    game_data = ServerGameDataFactory(tag='foobar',
                                      port=10480,
                                      gametype='VIP Escort',
                                      players=[PlayerGameDataFactory(name='Serge',
                                                                     loadout__primary=10,
                                                                     weapons__kills=9000)])
...
@pytest.mark.parametrize('stream_ip, stream_port, server_id', [
