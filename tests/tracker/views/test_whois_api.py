...
def test_unregistered_server_is_not_permitted_to_use_whois_api(client):
    response = client.get(
        "/api/whois/?0=foo&1=whois&2=ham&3=Serge%091.1.10.20", HTTP_X_REAL_IP="192.168.1.125"
    )
    assert_error_code(response)
...
def test_unlisted_server_is_not_permitted_to_use_whois_api(client):
    ...
    response = client.get(
        "/api/whois/?0=foo&1=whois&2=ham&3=Serge%091.1.10.20", HTTP_X_REAL_IP="127.0.0.5"
    )
    assert_error_code(response)
...
def test_whois_match_country_isp(client, whois_mock, name, ip, country, isp):
    response = client.get(
        f"/api/whois/?0=foo&1=whois&2=ham&3={name}%09{ip}", HTTP_X_REAL_IP="127.11.12.44"
    )
    assert_success_code(response)
...
def test_whois_match_profile(client, whois_mock, name, ip, alias, server):
    ...
    assert messages[0].startswith(
        f"[c=00FF00][b]{ip}[\\b][\\c] belongs to [c=00FF00]Cyprus[\\c] (Epic)"
    )
    ...
    if not has_alias:
        ...
        assert (
            messages[1]
            == f"[c=00FF00][b]{name}[\\b][\\c] is better known as [c=00FF00][b]{alias}[\\b][\\c]"
        )

    if has_server:
        assert (
            messages[-1]
            == f"[c=00FF00][b]{alias}[\\b][\\c] was last seen on [c=00FF00]{server}[\\c] now"
        )
