...
from apps.tracker.factories import (ServerFactory, ListedServerFactory,
                                    ServerQueryFactory)
...
def test_hostname_not_updated_if_not_changed(server, django_assert_num_queries):
    servers = {
        server: {'hostname': 'foo'}
    }
...
def test_offline_servers_detected_is_called_if_hit_failure_limit(db):
    ...
    with mock.patch.object(offline_servers_detected, 'send', wraps=offline_servers_detected.send) as signal_mock:
...
def test_offline_servers_are_removed_from_redis(db, redis):
    ...
    redis.hmset('servers', {'10.20.30.40:10480': '{}',
                            '10.20.30.40:10580': '{}',
                            '10.20.30.40:10680': '{}'})
    offline_servers_detected.send(sender=None, servers=servers)
