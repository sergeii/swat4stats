...
def test_serverquery_async_task_pool(create_udpservers):
    with create_udpservers(3) as udp_servers:
        ...
        udp_servers[2].responses.append(ServerQueryFactory(hostname='-==MYT Team Svr==-',
                                                           gametype='VIP Escort',
                                                           with_players_count=15).as_gamespy())

        aio.run_many(tasks)

        assert isinstance(result[servers[0]], asyncio.TimeoutError)
        assert isinstance(result[servers[1]], ServerStatusTask.ResponseMalformed)
...
def test_adminmod_serverquery_is_supported(udp_server):
    ...
    assert data['hostname'] == ('[C=FF0000][c=33CCCC]>|S[C=FFFFFF]S|<[c=ffff00]Arg[C=ffffff]'
...
def test_response_malformed(udp_server):
    ...
    assert isinstance(data, ServerStatusTask.ResponseMalformed)
