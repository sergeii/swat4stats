...
from tests.factories.tracker import MapFactory, PlayerFactory, ProfileFactory
...
def test_fill_stats(db, settings):
    ...
    for model in [PlayerStats, GametypeStats, ServerStats]:
        profile1_positions = set(
            model.objects.filter(profile=profile1).values_list("position", flat=True)
        )
        assert profile1_positions == {1}
        profile2_positions = set(
            model.objects.filter(profile=profile2).values_list("position", flat=True)
        )
