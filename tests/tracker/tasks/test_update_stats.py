...
from apps.tracker.factories import ProfileFactory, PlayerFactory, PlayerStatsFactory, MapFactory, GametypeStatsFactory
...
def test_update_player_stats(db, abomb, brewer):
    ...
    PlayerFactory(alias__profile=profile1,
                  ...
                  score=20,
                  time=50)
    PlayerStatsFactory(profile=profile1,
                       year=2021,
                       category='time',
                       points=1510,
                       position=1)
    ...
    PlayerStatsFactory(profile=profile1,
                       ...
                       points=192,
                       position=18)
    GametypeStatsFactory(profile=profile1,
                         gametype='VIP Escort',
                         year=2022,
                         category='score',
                         points=1)
    ...
    assert MapStats.objects.get(year=2022, profile=profile1, map=abomb, category='score').points == 192
    assert MapStats.objects.get(year=2022, profile=profile1, map=brewer, category='score').points == 20
    assert MapStats.objects.filter(~Q(year=2022), profile=profile1).count() == 0

    assert GametypeStats.objects.get(year=2022, profile=profile1, gametype='VIP Escort', category='score').points == 212
    ...
    assert MapStats.objects.get(year=2022, profile=profile2, map=abomb, category='score').points == 43
    ...
    assert GametypeStats.objects.get(year=2022,
                                     profile=profile2,
                                     gametype='Barricaded Suspects',
                                     category='score').points == 18
    assert GametypeStats.objects.get(year=2022,
                                     profile=profile2,
                                     gametype='Rapid Deployment',
                                     category='score').points == 25
    assert GametypeStats.objects.filter(~Q(gametype__in=['Barricaded Suspects', 'Rapid Deployment']),
                                        year=2022,
                                        profile=profile2).count() == 0
...
def test_update_player_stats_no_updates(db):
    ...
    emptied_profile = ProfileFactory(first_seen_at=datetime(2019, 1, 19, tzinfo=UTC),
                                     last_seen_at=datetime(2022, 8, 29, tzinfo=UTC),
                                     stats_updated_at=then - timedelta(days=3))
    PlayerFactory(alias__profile=emptied_profile,
                  game__date_finished=datetime(2020, 11, 3, tzinfo=UTC),
                  common=True)

    not_updated_emptied_profile = ProfileFactory(first_seen_at=then - timedelta(days=13),
                                                 last_seen_at=then - timedelta(days=1))
    ...
    PlayerFactory(alias__profile=updated_profile,
                  game__date_finished=then,
                  common=True)
...
def test_update_player_stats_for_last_played_year(db):
    ...
    PlayerFactory(alias__profile=old_profile,
    ...
    PlayerFactory(alias__profile=recent_profile,
                  game__date_finished=datetime(2021, 8, 16, 18, 18, 31, tzinfo=UTC),
                  kills=5)
    PlayerStatsFactory(profile=recent_profile,
                       year=2021,
                       category='kills',
                       points=3,
                       position=99)
    ...
    assert PlayerStats.objects.filter(~Q(year__in=[2021, 2022]), profile=recent_profile).count() == 0
    ...
    assert PlayerStats.objects.get(year=2021, profile=recent_profile, category='kills').position == 99
    assert PlayerStats.objects.filter(year=2021, profile=recent_profile, category='score').count() == 0
    ...
    assert PlayerStats.objects.get(year=2022, profile=recent_profile, category='kills').position == 3
    assert PlayerStats.objects.get(year=2022, profile=recent_profile, category='score').points == 29
    assert PlayerStats.objects.get(year=2022, profile=recent_profile, category='score').position is None
