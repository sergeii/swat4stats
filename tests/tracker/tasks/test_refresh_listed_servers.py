...
from apps.tracker.tasks import refresh_listed_servers, refresh_servers_chunk
...
def test_server_status_signals_are_invoked(db, create_udpservers):
    with create_udpservers(5) as udp_servers:
        ...
        failing_server.failures = settings.TRACKER_STATUS_FAILURES - 1
        failing_server.save()

        with create_live_mock() as live_mock, \
        ...
        assert Server.objects.filter(pk__in=[servers[4].pk], failures=settings.TRACKER_STATUS_FAILURES).count() == 1
