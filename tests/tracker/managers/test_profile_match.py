...
def test_match_smart_prefers_name_ip_to_name_isp(db):
    ...
    assert Profile.objects.match_smart(name='Serge', ip_address='127.0.0.1', isp=isp2).pk == profile1.pk
    assert Profile.objects.match_smart(name='Serge', ip_address='192.168.1.10', isp=isp1).pk == profile2.pk
...
def test_match_smart_will_not_match_other_profiles_with_null_isp(db):
    ...
    assert Profile.objects.match_smart(name='Serge', isp=ISPFactory(), ip_address='127.0.0.1') == profile1
    assert Profile.objects.match_smart(name='Serge', isp=None, ip_address='127.0.0.2') == profile2
    assert Profile.objects.match_smart(name='Serge', isp=ISPFactory(), ip_address='127.0.0.2') == profile2
